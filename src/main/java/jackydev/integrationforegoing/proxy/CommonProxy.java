package jackydev.integrationforegoing.proxy;

import jackydev.integrationforegoing.Config;
import jackydev.integrationforegoing.IntegrationForegoing;
import jackydev.integrationforegoing.compat.IFBookCompat;
import jackydev.integrationforegoing.compat.TConstructCompat;
import jackydev.integrationforegoing.handler.bioreactor.*;
import jackydev.integrationforegoing.handler.laserdrill.*;
import jackydev.integrationforegoing.handler.plant.extrautils2.PlantRecollectableExtraUtilities2;
import jackydev.integrationforegoing.handler.plant.immersiveengineering.PlantRecollectableIE;
import jackydev.integrationforegoing.handler.plant.mysticalagradditions.PlantRecollectableMysticalAgradditions;
import jackydev.integrationforegoing.handler.plant.oreberries.PlantRecollectableOreberries;
import jackydev.integrationforegoing.handler.plant.oreshrubs.PlantRecollectableOreShrubs;
import jackydev.integrationforegoing.handler.straw.immersiveengineering.StrawRegistryIE;
import jackydev.integrationforegoing.handler.straw.tconstruct.StrawRegistryTCon;
import jackydev.integrationforegoing.handler.straw.thermalfoundation.StrawRegistryTF;
...
public class CommonProxy {
	...
	public void init(FMLInitializationEvent event) {
        ...
        if (Config.harvestCraftIntegration) {
            if (Loader.isModLoaded("harvestcraft")) {
                IntegrationForegoing.logger.info("Registering Bioreactor entries for Pam's HarvestCraft...");
                BioReactorHandlerHarvestCraft.init();
                IntegrationForegoing.logger.info("Registered Bioreactor entries for Pam's HarvestCraft");
            }
        }
	}
